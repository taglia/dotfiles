...
for (const provider of ["fixture", "openai-codex"])
  test(
    `real SDK worker (${provider}): auth snapshot, no discovery, guarded tools and result`,
    ...
    async (t) => {
      ...
      const server = createServer(async (req, res) => {
        ...
        requests.push({ ...request, _headers: req.headers });
      ...
      const worker = startWorker({
        ...
        payload: {
          ...
          auth: {
            ...
            headers: { "X-Literal-Credential": "!printf wrong $NOT_ENV" },
          },
          model: {
            id: "cheap",
            name: "Cheap",
      ...
      assert.ok(requests.every((r) => r._headers.authorization === "Bearer fixture-secret"));
      assert.ok(
        requests.every((r) => r._headers["x-literal-credential"] === "!printf wrong $NOT_ENV"),
      );
