...
- Polls every `MONITOR_INTERVAL_MS` (15s).
...
`monitor.ts` top: `MONITOR_INTERVAL_MS`, `STALL_THRESHOLD_MS`, `TAIL_LINES`.
`tools.ts` top: `GRACE_MS`. `registry.ts` top: `MAX_LOG_BYTES`.